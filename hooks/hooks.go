...
func isGoFishHook(path string) (bool, error) {
	f, err := os.Open(path)
	if err != nil {
		return false, errors.Wrapf(err, "Failed to read hook file %s", path)
	}
	defer f.Close()
...
func createHook(name, path, script string, force bool) error {
	if util.FileOrDirExists(path) {
		isGoFish, err := isGoFishHook(path)
		if err != nil {
			return errors.Wrapf(err, "Failed to read hook %s", name)
		} else if !(isGoFish || force) {
